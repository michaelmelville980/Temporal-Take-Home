...
from activities import ReceiveOrder, ValidateOrder, ChargePayment
...
class OrderWorkflow:
    @workflow.run
    async def run (self, order_id: str, payment_id: str, items: List[Dict[str, Any]]):
        ...
        await workflow.execute_child_workflow(ShippingWorkflow.run, args=[order_id], task_queue="shipping-tq")
